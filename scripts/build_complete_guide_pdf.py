...
def build():
    ...
    story += [
        ...
        P("NLP layer (Qwen 3.8 27B via Groq)", "H2"),
        P("The NLP that feeds the stamp is <b>Qwen 3.8 27B</b> on Groq (model id <font face='Courier'>qwen/qwen3.8-27b</font>). It reads subject + body wording and we map the class to bounded points: clean +0, urgency +12, payment +16, credential +18. Header rules still run. Sklearn TF-IDF is fallback if there is no key or Groq is down."),
        P("Free Groq key: https://console.groq.com/keys then:"),
        Preformatted("python scripts\\set_groq_key.py", S["Cmd"]),
        P("That writes GROQ_API_KEY to a git-ignored .env and sets MAILTRACE_LLM_ENABLED=1. Never commit the key."),
    ...
    story += [
        ...
        *shot("case_nlp.png", 42 * mm),
        P("Figure 4 — NLP in the score path. Groq Qwen 3.8 27B when a key is set; sklearn if not.", "Cap"),
    ...
    terms = [
        ...
        ("NLP", "Groq Qwen 3.8 27B classifies subject/body wording and adds bounded points (0 / 12 / 16 / 18). Sklearn TF-IDF is fallback with no Groq key."),
        ("Qwen / Groq", "Free-tier key from console.groq.com. This is the NLP layer. Also writes sidebar notes. Redacted input. Not a probability."),
    ...
    story += [
        ...
        P("<b>Pipeline:</b> .eml bytes → parse_eml → offline WHOIS → optional live SPF/DMARC TXT → Qwen NLP wording class (or sklearn fallback) → fuse (rules + bounded NLP points) → SQLite + SHA-256 → graph → PDF."),
    ...
    stack = [
        ...
        ["NLP", "Groq Qwen 3.8 27B", "Wording class → bounded score points; sklearn fallback"],
        ...
        ["Assist", "Same Groq Qwen call", "Sidebar notes plus NLP class"],
    ...
    story += [
        ...
        P("1. “This is MailTrace. Offline email forensics. No Gmail.”<br/>2. Open 04 → 64 SPOOF. Point at SPF/DKIM/DMARC fail and Frankfurt hop (hosting, not a person).<br/>3. Open 01 → 8 CLEAN. Same engine, honest low score.<br/>4. Then 07 then 08 → two nodes, one edge. Campaign candidate.<br/>5. Evidence / PDF → hashed original bytes.<br/>6. Close: “Rules still score auth/hops. NLP is Qwen 3.8 27B via Groq, bounded points. Geo is a server.”"),
